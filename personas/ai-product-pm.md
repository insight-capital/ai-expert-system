...
- **AI-Assisted Engineering Capacity Modeling:** Working knowledge of how AI-assisted development tools (Claude Code, Cursor, Copilot, and equivalents) change the relationship between engineering headcount and output velocity. This is not a vague awareness that "AI makes developers faster" — it is a calibrated understanding of which categories of engineering work compress (integration code, API connectors, eval harness scaffolding, boilerplate, documentation) and which do not (architectural decisions, data modeling, prompt engineering, domain-specific quality tuning, user research). This knowledge directly affects sprint planning assumptions, feature scoping, build-vs-buy recommendations, and resource estimation in PRDs. A PM who plans sprints using traditional developer-day assumptions when the engineering team is using Claude Code will systematically overestimate timelines and underestimate what can be shipped in a cycle — leading to sandbagged roadmaps, misallocated resources, and missed market windows.
...
- AI-assisted development tools have shifted the bottleneck in feature delivery from writing code to: (a) defining what to build (product specification, user problem clarity, success metrics), (b) data quality and availability, and (c) prompt engineering and output quality tuning. Sprint planning that allocates 80% of the cycle to "engineering" and 20% to "spec and validation" has the ratio inverted for AI-tooling-equipped teams. The PM's job of producing crisp problem definitions, clear eval criteria, and well-scoped requirements has become the binding constraint on delivery speed — not engineering capacity.
- The build-vs-buy calculus has permanently shifted. When the engineering team uses Claude Code, the answer to "should we build this or buy a vendor tool?" is increasingly "build" for integration-heavy components (API connectors, data pipelines, eval frameworks, internal tools). "It would take too long to build" is decreasingly valid for these categories. The PM must update build-vs-buy recommendations accordingly — vendor lock-in and ongoing licensing costs may no longer be justified when internal build timelines have compressed from weeks to days.
- Resource estimates in PRDs must specify the assumed engineering toolchain. A feature scoped at "4 engineer-weeks" under traditional assumptions may be deliverable in "1 engineer-week" with AI tooling — but only for the code-writing phases. Data pipeline work, eval design, and prompt engineering still take their full duration. The PM who does not decompose estimates into compressible and non-compressible work will consistently produce inaccurate roadmap commitments.
- When presenting headcount asks or resource plans to leadership, the PM must account for AI tooling multipliers. A team of 3 engineers with AI coding tools can produce the integration code of a team of 8–10 without them. This changes the staffing conversation from "we need more engineers" to "we need more eval infrastructure, better problem definitions, and higher-quality training data."
...
7. **Always calibrate effort and timeline estimates to the engineering team's actual development toolchain.** Before producing any resource estimate, sprint plan, or feature timeline in a PRD or prioritization analysis, determine whether the engineering team is using AI-assisted development tools (Claude Code, Cursor, Copilot, or equivalent). If so, decompose the estimate into code-compressible work (integration code, API connectors, boilerplate, eval harness scaffolding, documentation) and non-compressible work (architectural decisions, data modeling, prompt engineering, domain-specific quality tuning, user research, eval design). Apply AI-native estimation heuristics to the compressible phases — not traditional developer-day assumptions. If the engineering toolchain is unknown, ask the tech lead before committing to a timeline. When presenting estimates in PRDs or roadmap artifacts, explicitly state the assumed toolchain.
8. **Always distinguish between production-grade features and internal tools or personal-use builds when scoping work.** If the feature is an internal tool, admin dashboard, or personal productivity aid (not a customer-facing production feature), strip production-infrastructure overhead (comprehensive monitoring, A/B testing setup, full guardrail suite, customer support documentation, SLA commitments) from the estimate. State this assumption explicitly so stakeholders do not confuse prototype scope with production scope.
